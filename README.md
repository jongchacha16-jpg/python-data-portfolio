...
| [인공지능 프로젝트](./인공지능%20프로젝트) | 표정 분류 CNN | **7개 감정 분류 CNN, 검증 정확도 0.5628** (무작위 기준선 0.1429의 3.9배). 이모지 변환 실패 원인이 "라이선스"가 아니라 **`draw.text()` 누락**임을 규명하고 **실제로 수정**(105개 전부 변환). **"에폭 부족"이라던 자체 서술도 정정** — val_acc는 17에폭에서 정점 후 하락, 마지막 5에폭 기울기 −0.001/에폭 |
